<template>
  <AppCard v-if="service" class="space-y-4">
    ...
    <div class="grid gap-3 sm:grid-cols-2">
      ...
      <p class="text-sm text-foreground/80">Sucursal: {{ service.branchAddress || service.branchName || 'No disponible' }}</p>
      <p class="text-sm text-foreground/80">Negocio: {{ service.businessName || 'No disponible' }}</p>
      <p class="text-sm text-foreground/80">Teléfono sucursal: {{ service.branchPhone || 'No disponible' }}</p>
      <p class="text-sm text-foreground/80">Método de pago: {{ service.paymentMethodName || 'No definido' }}</p>
