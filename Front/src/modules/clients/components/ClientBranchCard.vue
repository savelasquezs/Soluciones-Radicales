<template>
	<article class="rounded-2xl border border-border bg-surface p-4">
		...
		>
			<div class="space-y-1">
				...
				<p class="text-sm text-foreground/65">
					{{ item.branch.city || 'Sin ciudad' }}
				</p>
				<p class="text-sm text-foreground/65">
					{{ item.branch.phone || 'Sin telefono' }}
				</p>
			</div>
			...
			<div class="flex flex-wrap gap-2">
				<AppButton
					variant="secondary"
					@click="$emit('edit-branch', item.branch)"
				>
					Editar sucursal
				</AppButton>
				<AppButton
					variant="secondary"
					@click="$emit('edit-config', item.branch)"
				>
					Configuracion
				</AppButton>
		...
		<div class="mt-4 grid gap-3 sm:grid-cols-2">
			<div>
				<p class="text-xs uppercase tracking-wide text-foreground/55">
					Precio por m2
				</p>
				<p class="text-sm text-foreground">
					{{
						item.branch.pricePerM2 !== null
							? formatCurrencyCOP(item.branch.pricePerM2)
							: '-'
					}}
				</p>
			</div>
			<div>
				...
				<p class="text-sm text-foreground">
					{{
						item.branch.fixedPrice !== null
							? formatCurrencyCOP(item.branch.fixedPrice)
							: '-'
					}}
				</p>
			</div>
			<div>
				...
				<p class="text-sm text-foreground">
					{{ item.branch.frequencyDays ?? '-' }} dias
				</p>
			</div>
			<div>
				<p class="text-xs uppercase tracking-wide text-foreground/55">
				...
				<p class="text-sm text-foreground">
					{{ item.branch.reinforcementDays ?? '-' }} dias
				</p>
			</div>
			<div>
				<p class="text-xs uppercase tracking-wide text-foreground/55">
				...
				<div class="flex gap-2 pt-1">
					<AppBadge
						:tone="item.branch.reinforcementEnabled ? 'success' : 'default'"
					>
						{{
					...
					<AppBadge
						:tone="item.branch.reinforcementIsPaid ? 'default' : 'success'"
					>
			...
			<div>
				...
				<p class="text-sm text-foreground">
					{{ item.branch.technicianRevenueMode }}
				</p>
			</div>
			<div>
				<p class="text-xs uppercase tracking-wide text-foreground/55">
				...
				<p class="text-sm text-foreground">
					{{
						item.serviceCycle?.nextMainServiceDate
							? formatDate(item.serviceCycle.nextMainServiceDate)
							: 'No programado'
					}}
				</p>
			</div>
			<div>
				...
				<p class="text-sm text-foreground">
					{{
						item.serviceCycle?.nextReinforcementDate
							? formatDate(item.serviceCycle.nextReinforcementDate)
							: 'No programado'
					}}
				</p>
			</div>
		...
		<AppModal :open="isCycleModalOpen" size="sm" @close="closeCycleEditor">
			<div class="space-y-5">
				<div>
					...
					<p class="text-sm text-foreground/65">
						Define proximo servicio y refuerzo de la sucursal.
					</p>
				</div>
				...
				<div class="space-y-4">
					<div class="space-y-2">
						<p class="text-sm font-medium text-foreground">
							Proximo servicio principal
						</p>
						<AppDatePicker
							v-model="nextMainDate"
							enableTimePicker
				...
				<div class="flex justify-end gap-2">
					<AppButton variant="secondary" @click="closeCycleEditor"
						>Cancelar</AppButton
					>
...
const saveCycleDates = () => {
	...
	if (
		reinforcementDateIso &&
		new Date(reinforcementDateIso) < new Date(mainDateIso)
	) {
		modalError.value =
			'La fecha de refuerzo no puede ser anterior al servicio principal.';
		return;
	}
