...
describe('auth.store', () => {
  ...
  it('login fallido setea error y no deja sesion autenticada', async () => {
    ...
    await expect(store.login('a@a.com', 'bad')).rejects.toEqual({ message: 'Credenciales inválidas' });
  ...
  it('logout limpia sesion aunque endpoint falle', async () => {
    const store = useAuthStore();
    store.user = { id: 'u1', name: 'Admin', email: 'a@a.com', role: 'admin', isTechnician: false };
  ...
  it('forgotPassword llama servicio y maneja loading/error', async () => {
    ...
    expect(authService.forgotPassword).toHaveBeenCalledWith({ email: 'admin@demo.com' });
    expect(store.isLoading).toBe(false);

    vi.mocked(authService.forgotPassword).mockRejectedValue({ message: 'Error' });
    await expect(store.forgotPassword('x')).rejects.toEqual({ message: 'Error' });
  ...
  it('resetPassword llama servicio y maneja loading/error', async () => {
    ...
    expect(authService.resetPassword).toHaveBeenCalledWith({ token: 't1', newPassword: '12345678' });
    expect(store.isLoading).toBe(false);

    vi.mocked(authService.resetPassword).mockRejectedValue({ message: 'Error' });
    await expect(store.resetPassword('t2', '12345678')).rejects.toEqual({ message: 'Error' });
    expect(store.error?.message).toBe('Error');
  });
});
