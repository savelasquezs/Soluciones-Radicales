...
describe('SettingsPage', () => {
  it('muestra cards Empresa, Métodos de pago y Usuarios y técnicos', () => {
    ...
    const usersCardLink = wrapper
      .findAll('a')
      .find((link) => link.text().includes('Usuarios y técnicos'));
    expect(usersCardLink?.attributes('href')).toBe('/settings/users');
  });
});
